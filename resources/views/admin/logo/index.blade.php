...
<div class="mb-3 text-end">
  ...
  <div class="card custom-card">
    <div class="card-header justify-content-between">
      <div class="card-title">DANH MỤC LOGO</div>
    </div>
    <div class="card-body">
      <div class="table-responsive theme-scrollbar" style="padding: 10px">
        <table class="display table table-bordered table-stripped text-nowrap datatable-custom122" id="datatable">
          ...
          <tbody>
            ...
            <tr>
                <td class="text-center">{{ $logos->name }}</td>
                <td class="text-center"><img src="{{ $logos->image }}" width="100px" style="border-radius: 10px;"></td>
                <td class="text-center">{{ number_format($logos->price)  }}đ</td>
                <td class="text-center">{{ $logos->ck }}%</td>
                <td class="text-center">{!! Helper::status_server_cron_admin($logos->status) !!}</td>
                <td class="text-center">
                  <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox"
                    role="switch"
                    id="flexSwitchCheckDefault{{ $logos->id }}"
                    value="{{ $logos->id }}"
                    onchange="updateStatus(this)"
                    @if($logos->status == 1) checked @endif>

                  <label class="form-check-label" for="flexSwitchCheckDefault{{ $logos->id }}"></label>
                   </div>
                </td>
                <td class="text-center">
                    <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#modal-update-{{ $logos->id }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-original-title="Edit">
                        <i class="fa-solid fa-pen-to-square"></i> Edit
                    </a>
                    <a href="javascript:deleteRow({{ $logos->id }})" class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="Delete">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{ route('admin.logo.update', ['id' => $value->id]) }}" class="default-form" method="POST" enctype="multipart/form-data">
              @csrf
              <div class="mb-3">
            ...
            <label for="images" class="form-label">Hình ảnh</label>
              ...
              <div class="mb-2 mt-2">
                <img src="{{ ($value->image) }}" alt="Logo" class="img-fluid" style="max-height: 100px;">
              </div>
             </div>
             <div class="mb-3 row">
               <div class="col-md-6">
  ...
  <div class="modal fade" id="modal-create" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Thêm logo mới</h5>
          <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form action="{{ route('admin.logo.store') }}" method="POST" class="default-form" data-reload="1" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
...
<script>
  const updateStatus = (element) => {
    ...
    axios.post(`/Cpanel/logo/update-status`, {
      id: id,
      status: !!status
    }).then((response) => {
      Swal.fire({
    ...
    const deleteRow = async (id) => {
      ...
      try {
        const {
          data: result
        } = await axios.post('{{ route('admin.logo.delete') }}', {
          id
        })
